...
d3.json("nsf_clustered_grants_d3_tagged.json").then(data => {
  ...
  const canonicalTagSelect = d3.select("#tagSelect");
  const canonicalTags = Array.from(new Set(data.flatMap(d => d.canonical_tags || []))).sort().slice(0, 15);
  canonicalTags.forEach(tag => canonicalTagSelect.append("option").attr("value", tag).text(tag));

  const keywordTagSelect = d3.select("#keywordTagSelect");
  const keywordTags = Array.from(new Set(data.flatMap(d => d.keyword_tags || []))).sort();
  keywordTags.forEach(tag => keywordTagSelect.append("option").attr("value", tag).text(tag));
      ...
      .on("mouseover", (event, d) => {
        tooltip.style("display", "block")
          ...
          .html(
            ...
            `<strong>Tags:</strong> ${d.canonical_tags?.join(", ") ?? "None"}<br>` +
            `<strong>Keyword Tags:</strong> ${d.keyword_tags?.join(", ") ?? "None"}`
          );
      })
  ...
  function update() {
    ...
    const canonicalTagVal = canonicalTagSelect.property("value");
    const keywordTagVal = keywordTagSelect.property("value");
    ...
    const filtered = data.filter(d => {
      ...
      const matchCanonicalTag = canonicalTagVal === "all" || (d.canonical_tags && d.canonical_tags.includes(canonicalTagVal));
      const matchKeywordTag = keywordTagVal === "all" || (d.keyword_tags && d.keyword_tags.includes(keywordTagVal));
      return matchCluster && matchQuery && matchState && matchInst && matchCanonicalTag && matchKeywordTag;
    });
  ...
  canonicalTagSelect.on("change", update);
  keywordTagSelect.on("change", update);
  update();
});
